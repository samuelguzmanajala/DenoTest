...
export class CreateUserCommandMother{
    static create(id: string, name: string, password: string, mail: string): CreateUserCommand{
        return new CreateUserCommand({id, name, password, mail});
    }
    static random(): CreateUserCommand{
        return this.create(UserIdMother.random().value, UserNameMother.random().value, UserPasswordMother.random().value, UserMailMother.random().value);
    }
}