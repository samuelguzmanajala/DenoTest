...
export async function getFilesList (directory: string, options: FilterOptions = {}):Promise<string[]>{
    const foundFiles: string[] = [];
    for await (const fileOrderFolder of Deno.readDir(directory)){
        if(fileOrderFolder.isDirectory){
            const nestedFiles = await getFilesList(path.join(directory, fileOrderFolder.name), options);
